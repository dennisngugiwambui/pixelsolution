...
async function loadProducts() {
    try {
        ...
        displayEmptyState('Failed to load products. Please check your connection and refresh.');
    }
}
...
function updateCartDisplay() {
    ...
    // If subtotal is the pre-tax amount, then total = subtotal / (1 - tax_rate)
    // But since you want tax to be 15% OF the final amount, we calculate:
    const total = subtotal; // The subtotal IS the total amount
    const tax = total * 0.15; // 15% of the total amount
    ...
    let cartHTML = '';
    cart.forEach(item => {
        // Get category-specific icon for cart item
        const defaultIcon = getCategoryIcon(item.categoryName);
...
async function completePayment() {
    ...
    if (cart.length === 0) {
        showToast('Cart is empty. Add products to make a sale.', 'error');
        return;
    }
    ...
    try {
        ...
        if (selectedPaymentMethod === 'cash') {
            ...
            console.log('💰 Cash payment validation:', {
                ...
                currentTotal,
                change,
                inputValue: cashReceivedInput.value
            });
            ...
            console.log('📱 MPESA payment validation:', {
                phone,
                phoneLength: phone.length
            });
            
            if (!phone || phone.length !== 9) {
                showToast('Please enter a valid 9-digit phone number (e.g., 712345678).', 'error');
                phoneInput.focus();
                return;
            }
        ...
        console.log('📤 Sale data stringified:', JSON.stringify(saleData, null, 2));
        ...
        console.log('📡 Request URL:', '/Admin/ProcessSale');
        ...
        try {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    const errorMessage = errorJson.message || errorJson.error || `HTTP ${response.status}: ${response.statusText}`;
            ...
            console.log('📥 Sale response stringified:', JSON.stringify(result, null, 2));
        } catch (parseError) {
        ...
        if (result.success) {
            ...
            console.log('✅ Sale completed:', result.saleNumber);
        } else {
            console.error('❌ Sale failed:', result.message);
            console.error('❌ Full sale response:', JSON.stringify(result, null, 2));
...
async function generateReceipt(saleId, saleNumber, totalAmount, changeGiven, cashierName) {
    try {
        console.log('🧾 Generating receipt for sale:', saleNumber);
...
async function attemptAutoPrint(receiptHtml, saleNumber) {
    return new Promise((resolve) => {
        try {
            ...
            printFrame.onload = () => {
                setTimeout(() => {
                    try {
                        ...
                        console.log('✅ Receipt printed successfully:', saleNumber);
                        ...
                        console.warn('⚠️ Print failed:', printError);
                        document.body.removeChild(printFrame);
                        resolve(false);
                    }
                }, 500);
            ...
            console.warn('⚠️ Auto-print setup failed:', error);
            resolve(false);
        }
    });
}
...
function setupEventListeners() {
    ...
    if (cashInput) {
        ...
        // Add Enter key support for cash payment
